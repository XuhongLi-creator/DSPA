...
class TriDet(nn.Module):
    ...
    def __init__(
            ...
            n_mmac_win_size,  # window size w for mmac
            ...
            init_conv_vars,  # initialization of gaussian variance for the weight in MMAC
        ...
        if isinstance(n_mmac_win_size, int):
            self.mmac_win_size = [n_mmac_win_size] * len(self.fpn_strides)
        else:
            assert len(n_mmac_win_size) == len(self.fpn_strides)
            self.mmac_win_size = n_mmac_win_size
        max_div_factor = 1
        for l, (s, w) in enumerate(zip(self.fpn_strides, self.mmac_win_size)):
        ...
        assert backbone_type in ['MMAC', 'conv']
        if backbone_type == 'MMAC':
            self.backbone = make_backbone(
                ...
                **{
                    ...
                    'mmac_win_size': self.mmac_win_size,
