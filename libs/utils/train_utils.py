...
from ..modeling import MaskedConv1D, Scale, AffineDropPath, LayerNorm,\
    Local_cnn, global_mask_atten, connectionBlock, Temporal_Merging_Block, TemporalEncoder, linearProj
...
def make_optimizer(model, optimizer_config):
    ...
    decay = set()
    no_decay = set()
    whitelist_weight_modules = (torch.nn.Linear, torch.nn.Conv1d, MaskedConv1D, linearProj)
    blacklist_weight_modules = (LayerNorm, torch.nn.LayerNorm, torch.nn.GroupNorm)
