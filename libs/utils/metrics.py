...
def load_pred_seg_from_json(json_file, label='label_id', label_offset=0):
    ...
    #print("json_db.items", json_db.items)
...
class ANETdetection(object):
    ...
    def evaluate(self, preds, verbose=True):
        ...
        if isinstance(preds, pd.DataFrame):
            ...
            preds_serializable = {key: value.tolist() if isinstance(value, np.ndarray) else value for key, value in preds.items()}
            ...
            #print(preds[preds['video-id'] == 'video_test_0000028'])
        # always reset ap
        self.ap = None
